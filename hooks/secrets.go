...
func (h *PreferParentSecretsHook) MutateCreatePhysical(
	...
	for i := range pod.Spec.Volumes {
		...
		translatedVolumeName := translate.PhysicalName(
			vPod.Spec.Volumes[i].VolumeSource.Secret.Name,
			vPod.Namespace,
		)

		if translatedVolumeName == volume.VolumeSource.Secret.Name {
			pVolumeName = vPod.Spec.Volumes[i].VolumeSource.Secret.Name
		}
		...
		volume.VolumeSource.Secret.Name = pVolumeName
	}
